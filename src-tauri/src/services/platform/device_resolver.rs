...
pub fn resolve_avf_indices() -> AppResult<AvfResolved> {
    ...
    println!(
        "[DeviceResolver] Resolving AVFoundation device indices using: {}",
    ...
    let parsed: AvfResolved = serde_json::from_str(stdout.trim()).map_err(|e| {
        AppError::Recording(format!(
    ...
    println!(
        "[DeviceResolver]   Built-in mic: {:?}",
        parsed.audio_index_builtin_mic
    );
    println!(
        "[DeviceResolver]   Built-in camera: {:?}",
        parsed.video_index_builtin_cam
    );
    println!(
        "[DeviceResolver]   Main screen: {:?}",
        parsed.video_index_main_screen
    );
    println!(
...
fn get_resolver_path() -> AppResult<PathBuf> {
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            ...
            if let Some(path) = alt_bundled_path {
                if path.exists() {
                    println!(
                        "[DeviceResolver] Found resolver in app bundle (alt): {:?}",
                        path
                    );
                    return Ok(path);
                }
    ...
    if let Ok(manifest_dir) = std::env::var("CARGO_MANIFEST_DIR") {
        ...
        if dev_path.exists() {
            println!(
                "[DeviceResolver] Found resolver in manifest dir: {:?}",
                dev_path
            );
            return Ok(dev_path);
        }
    ...
    if let Ok(cwd) = std::env::current_dir() {
        ...
        if cwd_path_alt.exists() {
            println!(
                "[DeviceResolver] Found resolver in CWD (alt): {:?}",
                cwd_path_alt
            );
    ...
    if absolute_path.exists() {
        println!(
            "[DeviceResolver] Found resolver at compile-time path: {:?}",
            absolute_path
        );
